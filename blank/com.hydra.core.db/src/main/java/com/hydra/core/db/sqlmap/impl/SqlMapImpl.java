...
public class SqlMapImpl implements SqlMap {
    ...
    public SqlMapImpl(DataSource dataSource, SqlSessionFactory sqlMapClient,
    ...
    public SqlMapSession startSession(boolean batch) {
        SqlSession session = batch ? createSession(dataSource, ExecutorType.BATCH)
                : createSession(dataSource, cacheStatement ? ExecutorType.REUSE : ExecutorType.SIMPLE);
    ...
    private SqlSession createSession(DataSource src, ExecutorType type) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("Opened SqlSession [" + session + "] for iBATIS operation");
        }
